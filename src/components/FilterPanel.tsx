    ...
    <>
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          className={`relative flex items-center gap-2 px-4 py-2.5 rounded-full text-xs font-bold border-2 cursor-pointer transition-all ${
            count > 0
        ...
        >
          ...
          <span>Filters</span>
          {count > 0 && (
            <span className="w-5 h-5 bg-[#C9A227] text-[#2A0E0A] rounded-full text-[10px] font-black flex items-center justify-center">
              {count}
            </span>
          )}
        </button>
        ...
        <div className="relative">
          <button
            ...
            className={`flex items-center gap-1.5 px-4 py-2.5 rounded-full text-xs font-bold border-2 cursor-pointer transition-all ${
              filters.sort !== 'popular'
                ? 'bg-[#C9A227]/10 border-[#C9A227] text-[#2A0E0A]'
                : 'bg-white text-[#2A0E0A] border-[#2C1A17]/15 hover:border-[#C9A227]/40'
            }`}
          >
            <span>{SORT_OPTIONS.find(o => o.value === filters.sort)?.label || 'Sort'}</span>
            ...
            {showSortDropdown && (
              <>
                ...
                <motion.div
                  ...
                  className="absolute top-full left-0 mt-2 z-50 bg-white border border-[#2C1A17]/10 rounded-2xl shadow-xl overflow-hidden w-52"
                >
                  {SORT_OPTIONS.map(opt => (
                    <button
                      key={opt.value}
                      onClick={() => { update({ sort: opt.value }); setShowSortDropdown(false); }}
                      className={`w-full flex items-center justify-between px-4 py-2.5 text-xs font-semibold cursor-pointer hover:bg-[#FAF6F0] transition-colors ${
                        filters.sort === opt.value ? 'text-[#2A0E0A] font-bold' : 'text-[#2C1A17]/70'
                      }`}
                    >
                      {opt.label}
        ...
        {count > 0 && (
          <button
            onClick={handleReset}
            className="flex items-center gap-1 text-[10px] font-bold text-red-500 hover:text-red-700 px-3 py-2.5 rounded-full hover:bg-red-50 border border-transparent hover:border-red-100 cursor-pointer transition-all"
          >
        ...
        {isOpen && (
          <>
            ...
            >
              ...
              {/* Apply Button */}
              <div className="sticky bottom-0 bg-white border-t border-[#2C1A17]/10 p-4">
                <button
                  onClick={() => setIsOpen(false)}
                  className="w-full bg-[#2A0E0A] text-[#C9A227] font-bold py-4 rounded-full cursor-pointer hover:bg-[#401C16] transition-all active:scale-95"
                >
