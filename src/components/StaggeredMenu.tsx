...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = React.memo(({
  ...
  className,
  menuButtonColor = '#2C1717', // Dark brown default for closed button
  openMenuButtonColor = '#FAF8F5', // Light cream default for opened button
  accentColor = '#D4AF37', // Brand gold accent color
  ...
  return (
    ...
    >
      <div ref={preLayersRef} className="sm-prelayers" aria-hidden="true">
        {(() => {
          const raw = colors && colors.length ? colors.slice(0, 4) : ['#E7DCBA', '#A38848', '#5B3535'];
          let arr = [...raw];
          if (arr.length >= 3) {
      ...
      <header className="staggered-menu-header" aria-label="Main navigation header">
        <div className="sm-logo" aria-label="Logo">
          <button 
            onClick={() => {
              closeMenu();
              const homeItem = items.find(it => it.link === 'home');
              if (homeItem?.onClick) {
                homeItem.onClick();
              }
            }}
